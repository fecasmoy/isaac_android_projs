...
package org.fecasmoy.activitylifecycle.util;
...
public class Utils {
	...
	/**
	 * Helper method to print out the lifecycle state of each Activity. Note
	 * this has been wrapped in a Handler to delay the output due to overlaps in
	 * lifecycle state changes as one Activity launches another.
	 * 
	 * @link 
	 ...
	 * @param viewStatus
	 *            TextView to list out the status of all Activity classes
	 */
		...
		handler.postDelayed(new Runnable() {
			public void run() {
				...
				for (String key : mStatusTracker.keySet()) {
					sbStatus.insert(0,
							key + ": " + mStatusTracker.getStatus(key) + "\n");
				}
				if (viewStatus != null) {
